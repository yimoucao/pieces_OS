...
void* people_maker_entry(void* arg){
	...
	// start making people
	int status=0;
	while(time(0)-begining_time < simulation_time) // check time
	{
		sleep(p); // sleep p seconds;
		int from = rand() % n_floors; 
		int to = -1; // TODO: check errors;  Is there any better way?

		// obtain 2 different floor number
		do {
			to = rand() % n_floors;
		}while(to == from);

		unsigned long id = make_id(person_id_maker); // get an id from id maker
		double arrival_time = time(0); // TODO: get arrival time, time(0) return time_t
		Person* person = make_person(id, from, to, arrival_time); // make a person
		status = PersonQueue_add_person(personqueue, person); // add this person to global queue
		if(status <= 0){
...
void* elevator_entry(void* arg){
    ...
    int status = 0; // status for whether letting one person in fail or not
	while(time(0)-begining_time < simulation_time){
		status = letin_person(elevator, personqueue);
        if ( status == -1 ) // cannot get person
        {
            continue;
        }
		int n_transported = transport_person(elevator, personqueue, e, begining_time); // e is elevator speed
		l_num_people_finished += n_transported; // count how many people finished thread-locally
	}

    printf("[%f]: Stoping elevator[%lu]\n", (double)time(0)-begining_time, eid);
    PersonQueue_set_timesup(personqueue); // flag that time's up on the queue
    cv_signal(personqueue->cv); //signal others to stop waiting

    // update number of people finished in the global variable
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	//parse argumetns from the command line
	...
	// declaration of threads, global variable initializations and seed the random number generator
	int i=0;
